...
class TranslatorTest extends AnyFunSuite {
  ...
  def getTranslator(name: String) = name match {
    case "peewee" =>
      PeeweeTranslator(Target(Peewee("foo", "dir"), SQLite("db")))
    case "django" =>
      DjangoTranslator(Target(Django("foo", "bar", "baz"), SQLite("db")))
    case "sqlalchemy" =>
      SQLAlchemyTranslator(Target(SQLAlchemy("foo", "bar"), SQLite("db")))
    case "sequelize" =>
      SequelizeTranslator(Target(Sequelize("foo", "bar"), SQLite("db")))
    case "activerecord" =>
      ActiveRecordTranslator(Target(ActiveRecord("foo", "bar"), SQLite("db")))
    case _ => ???
  }
  ...
  def translateQuery(name: String, qid: Int, q: Query) = {
    ...
    val qStr = translator.constructQuery(s, first, offset, limit)
    val actualQStr = qStr.ret match {
      case None => qStr.toString
      case Some(ret) =>
        qStr.toString + "\n" + translator.emitPrint(q, dfields, ret)
    }
    val expectedQStr = Source
      .fromFile(
        getClass.getResource(name + "/query_" + (qid + 1) + ".out").getFile()
      )
      .mkString
  ...
  TestQueries.queries.zipWithIndex.foreach { case (q, i) =>
    ...
    test("testing SequelizeTranslator in query " + (i + 1)) {
      val invalidQueries = List(1, 2, 4, 6, 10, 16, 20, 23, 24, 28, 29, 32, 34,
        37, 38, 42, 44, 45, 58, 59, 63, 64, 66, 67, 68)
