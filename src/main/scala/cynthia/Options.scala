  ...
  val cliParser = new scopt.OptionParser[Options]("cynthia") {
    ...
    def randomSeed() =
      opt[Int]("random-seed")
        ...
        .text(
          "Make the testing procedure deterministic by giving a random seed"
        )
    ...
    cmd("test") action { (_, c) => c.copy(mode = Some("test")) } children (
      opt[Int]('n', "queries")
        ...
        .text(
          "Number of queries to generate for each schema (default value: 200)"
        )
        .validate(x =>
    ...
    cmd("replay") action { (_, c) => c.copy(mode = Some("replay")) } children (
      opt[String]('c', "cynthia")
        ...
        .text(
          "The cynthia directory for replaying missmatches (default value: .cynthia)"
        ),
      opt[String]('s', "schema")
    ...
    cmd("inspect") action { (_, c) =>
      c.copy(mode = Some("inspect"))
    } children (
      opt[String]('c', "cynthia")
        .action((x, o) => o.copy(dotCynthia = x))
        .text(
          "The cynthia directory for inspecting missmatches (default value: .cynthia)"
        ),
      opt[String]('s', "schema")
        .action((x, o) => o.copy(schema = Some(x)))
