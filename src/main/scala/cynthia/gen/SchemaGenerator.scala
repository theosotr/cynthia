...
object SchemaGenerator {
  ...
  def generateFields(schema: Schema) = {
      ...
      if (fields.size > lowerBound && RUtils.bool()) fields
        ...
        val res =
          RUtils.chooseFrom(
            ...
            case "int" =>
              Some(RUtils.word(special = false).toLowerCase, Int32, foreignF)
            case "numeric" =>
              Some(RUtils.word(special = false).toLowerCase, Numeric, foreignF)
            case "string" =>
              Some(
                RUtils.word(special = false).toLowerCase,
                VarChar(50),
                foreignF
              )
            case "foreign" => {
    ...
    _genFields(
      Seq(Field("id", Serial), Field("simvolosira", VarChar(50))),
      Set()
    ) // Every model has an ID and a string field.
  }
