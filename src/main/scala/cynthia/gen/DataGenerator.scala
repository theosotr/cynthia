  ...
  def dataToInsertStmts(m: Model, data: Seq[Seq[Constant]]) =
    data.foldLeft(
      ...
      acc <<
        ...
        (
          m.fields map (x =>
            Utils.quoteStr(Field.dbname(x), quotes = "\"")
          ) mkString ","
        ) <<
        ") VALUES (" <<
        (
  ...
  def generateRow(fields: Seq[Field], i: Int): Seq[Constant] =
    fields map { case Field(n, t) =>
      t match {
        ...
        case Foreign(_) =>
          Constant((RUtils.integer(n = foreignKeyCands) + 1).toString, UnQuoted)
      }
    }

  def generateModelData(model: Model) = {
    def _generateData(
        data: LazyList[Seq[Constant]],
        i: Int
    ): LazyList[Seq[Constant]] =
      if (i <= 0) data
...
case class SolverDataGenerator(
  ...
  def constructJoinVars(models: Seq[String], state: State) = {
    ...
    (models zip models.tail).foldLeft(Seq[String]()) { case (prev, (s, t)) =>
      schema.models get t match {
        case None =>
          throw new Exception(s"Model $t not found in schema '{$schema.name}'")
        case Some(m) => {
  ...
  def getVariableJ(field: String, i: Int, state: State) =
    field split '.' match {
  ...
  def constructExpr(e: FieldExpr, i: Int, state: State): Expr = e match {
    ...
    case Count(None) =>
      ctx.mkInt("1") // It's the number of records included in each group.
  ...
  def constructConstraint(
      k: String,
      e: FieldExpr,
      op: (Expr, Expr) => BoolExpr,
      state: State
  ) = {
  ...
  def predToFormula(pred: Predicate, state: State): BoolExpr = pred match {
    ...
    case Lt(k, e) =>
      constructConstraint(
        ...
        e,
        (e1, e2) =>
          ctx.mkLt(e1.asInstanceOf[ArithExpr], e2.asInstanceOf[ArithExpr]),
        state
      )
    case Lte(k, e) =>
      constructConstraint(
        ...
        e,
        (e1, e2) =>
          ctx.mkLe(e1.asInstanceOf[ArithExpr], e2.asInstanceOf[ArithExpr]),
        state
      )
    case Gt(k, e) =>
      constructConstraint(
        ...
        e,
        (e1, e2) =>
          ctx.mkGt(e1.asInstanceOf[ArithExpr], e2.asInstanceOf[ArithExpr]),
        state
      )
    case Gte(k, e) =>
      constructConstraint(
        ...
        e,
        (e1, e2) =>
          ctx.mkGe(e1.asInstanceOf[ArithExpr], e2.asInstanceOf[ArithExpr]),
        state
      )
    case Contains(k, v) =>
      constructConstraint(
        ...
        Constant(v, Quoted),
        (e1, e2) =>
          ctx.mkContains(e1.asInstanceOf[SeqExpr], e2.asInstanceOf[SeqExpr]),
        state
      )
    case StartsWith(k, v) =>
      constructConstraint(
        ...
        Constant(v, Quoted),
        (e1, e2) =>
          ctx.mkPrefixOf(e2.asInstanceOf[SeqExpr], e1.asInstanceOf[SeqExpr]),
        state
      )
    case EndsWith(k, v) =>
      constructConstraint(
        ...
        Constant(v, Quoted),
        (e1, e2) =>
          ctx.mkSuffixOf(e2.asInstanceOf[SeqExpr], e1.asInstanceOf[SeqExpr]),
        state
      )
  ...
  def getFieldConstraints(m: Model, state: State, prefix: String = "") =
    m.fields.foldLeft(Seq[BoolExpr]()) {
      case (acc, Field(f, t)) => {
        ...
        if (
          !state.getNonConstantGroupingFields().contains(varName) || f.equals(
            "id"
          )
        ) {
  ...
  def generateData() = solver.check match {
    case Status.SATISFIABLE => {
      ...
      models.foldLeft(Map[Model, Seq[Seq[Constant]]]()) {
        case (acc, x) => {
          ...
          val records = List.range(0, nuRecords) map { i =>
            (model.fields map {
              ...
              case (_, (exprs, StringF)) =>
                Constant(
                  m.eval(exprs(i), true).toString.replace("\"", ""),
                  Quoted
                )
              case (_, (exprs, _)) =>
                Constant(m.eval(exprs(i), true).toString, UnQuoted)
            }).toSeq
          }
          acc + (model -> records)
        }
    ...
    def inspectExpr(e: FieldExpr): Unit = e match {
      case F(f) =>
        f split '.' match {
          ...
          case arr =>
            constructJoinVars(
              arr.dropRight(1).toSeq map { _.capitalize },
              state
            )
        }
      ...
      genStrategy match {
        ...
        case Some(genStrategy) => {
          ...
          if (data.isEmpty) DataGenFailed
            ...
            val insStms =
              data
                .foldLeft(Str("")) { case (acc, (model, data)) =>
