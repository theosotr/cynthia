...
case object FieldFilterer {
  ...
  def filterModelField(fType: Option[FType], filterForeign: Boolean = false)(
      f: Field
  ) = fType match {
    case None => true
    case Some(NumberF) =>
...
case class GenState(
  ...
  def getFields(declaredOnly: Boolean, fType: Option[FType]) = {
    val declFields =
      dfields filter FieldFilterer.filterDeclaredField(fType) filter {
        !FieldDecl.hidden(_)
      } map FieldDecl.as
    if (declaredOnly) declFields
    else
      declFields ++
        (model.get.fields filter FieldFilterer.filterModelField(
          fType,
          filterForeign = true
        ) map { x =>
          x match {
...
case class QueryGenerator(
  ...
  def getColumn(
    ...
    val field = RUtils.chooseFrom(
      model.fields filter FieldFilterer.filterModelField(fType)
    )
    ...
    field match {
      case Field(_, Serial) | Field(_, Int8) | Field(_, Int16) |
          Field(_, Int32) | Field(_, Int64) =>
        (fieldName, IntF)
  ...
  def chooseField(
    ...
    if (declaredOnly || (!s.dfields.isEmpty && RUtils.bool())) {
      val typedDFields =
        s.dfields filter FieldFilterer.filterDeclaredField(fType)
      val dfields =
  ...
  def generateFieldExpr(
    ...
    val exprNode =
      ...
      else if (!forAggr && (s2.depth < minDepth)) {
        val fExprs = s2.exprCands filter (e =>
          (fType, e) match {
            case (None, FExpr) | (None, ConstantExpr) | (Some(NumberF), FExpr) |
                (Some(NumberF), ConstantExpr) =>
              false
    ...
    exprNode match {
      ...
      case AddExpr => {
        val e1 = generateFieldExpr(
          s2 ++,
          model,
          fType,
          declaredOnly = declaredOnly,
          nonAggrField = nonAggrField
        )._1
        val e2 = generateFieldExpr(
          s2 ++,
          model,
          fType,
          declaredOnly = declaredOnly,
      ...
      case MulExpr => {
        ...
        val e2 =
          generateFieldExpr(s2 ++, model, fType, declaredOnly = declaredOnly)._1
        (Mul(e1, e2), DoubleF)
      }
      case DivExpr => {
        val e1 = generateFieldExpr(
  ...
  def generatePredicate(
    ...
    predNode match {
      ...
      case StartsWithPred =>
        StartsWith(RUtils.chooseFrom(fields), RUtils.word())
      case EndsWithPred => EndsWith(RUtils.chooseFrom(fields), RUtils.word())
      case AndPred =>
        And(
          generatePredicate(s2 ++, model, fType),
          generatePredicate(s2 ++, model, genQSType())
        )
      case OrPred =>
        Or(
          generatePredicate(s2 ++, model, fType),
  ...
  def generateDeclFields(
    ...
    def _generateFieldDecl(
        s: GenState,
        declF: Option[List[FType]]
    ): GenState = {
      val (stopCond, fields) =
          ...
          declF match {
            case None =>
              (
                RUtils.bool(),
                None
              ) // We randomly choose whether to stop or not.
            case Some(Nil) =>
              (true, None) // We do not have more fields to generate.
            case _ => (false, declF)
          }
      if (stopCond) s
      else {
  ...
  def generateQuerySetforCombined(s: GenState) = s.qs match {
    ...
    case None => {
      ...
      val dFields = List.range(0, nuf) map { _ =>
        RUtils.chooseFrom(Seq(NumberF, StrF))
      }
      (
        // first sub-query
        generateQuerySet(
  ...
  def generateQuerySet(
      s: GenState,
      declF: Option[List[FType]] = None,
      hiddenF: Boolean = false,
      declaredOnly: Boolean = false
  ): GenState =
    ...
    if (stopGen(s)) s
      ...
      next match {
        case NewQS => {
          ...
          val s2 = genDefaultField(
            generateDeclFields(s ++, model, nonHidden = hiddenF, declF = declF),
            model
          )
        ...
        case UnionQS => {
          ...
          generateQuerySet(
            combined candidates List(ApplySort, UnionQS, IntersectQS),
            declaredOnly = true
          )
        }
        case IntersectQS => {
  ...
  def apply(schema: Schema): Query = {
    ...
    qType match {
      case "set" =>
        SetRes(
          generateQuerySet(
            GenState(schema, cands = cands, exprCands = exprNodes)
          ).qs.get
        )
      case "first" => {
        val qs = generateQuerySet(
          GenState(schema, cands = cands, exprCands = exprNodes)
        ).qs.get
      ...
      case "aggr" => {
        val exprCands = exprNodes filter { x =>
          x match {
