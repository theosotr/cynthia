...
case class InspectRes(
  ...
  def addCrash(crash: Crash, queryId: Int) =
    crashes get crash match {
      case None => InspectRes(crashes + (crash -> List(queryId)), queryRes)
      case Some(queries) =>
        InspectRes(crashes + (crash -> (queries :+ queryId)), queryRes)
    }

  def addQueryRes(queryOut: QueryResult, orm: String) =
    queryRes get queryOut match {
      case None => InspectRes(crashes, queryRes + (queryOut -> List(orm)))
      case Some(orms) =>
        InspectRes(crashes, queryRes + (queryOut -> (orms :+ orm)))
    }
}
...
case object InspectRes {
  ...
  def printMismatches(inspectRes: InspectRes, startIdent: Int) = {
    ...
    (inspectRes.queryRes.foldLeft(Map[(Int, String), Seq[Seq[String]]]()) {
      case (acc, (k, m)) =>
      ...
      case ((query, backend), mis) => {
        dump(
          s"${Console.CYAN}* ${query.toString}[${backend}]: ${Console.RESET}",
          ident = startIdent + 1
        )
        mis foreach { x =>
  ...
  def printCrashes(inspectRes: InspectRes, startIdent: Int) = {
    ...
    (inspectRes.crashes.foldLeft(
      Map[(String, String), Seq[(String, Seq[Int])]]()
    ) { case (acc, (k, q)) =>
      ...
      dump(
        s"${Console.CYAN}* ${k._1}[${k._2}]:${Console.RESET}",
        ident = startIdent + 1
      )
      v foreach {
        case (desc, ids) => {
          dump(
            "- " + desc + ": Queries(" + ids.mkString(",") + ")",
            ident = startIdent + 3
          )
        }
      }
...
object Inspector {
  ...
  def inspectMismatch(dir: String, res: InspectRes) = {
    Utils.listFiles(dir, ext = ".out") match {
      ...
      case Some(outputs) =>
        (outputs
          .foldLeft(Map[String, List[String]]()) { (acc, x) =>
            {
              val Array(orm, backend) =
                new File(x).getName.replace(".out", "") split '_'
          ...
          .foldLeft(res) { case (acc, (k, v)) =>
            v.foldLeft(acc) { case (res, orm) =>
              inspectContent(
                Utils.readFromFile(
                  Utils.joinPaths(List(dir, orm + "_" + k + ".out"))
                ),
                orm,
                k,
  ...
  def apply(
    ...
    val dir = Utils.joinPaths(List(workdir, Utils.sessionDir, schema))
    Utils.listFiles(dir, ext = "") match {
      ...
      case Some(queries) => {
        ...
        val q = mismatches match {
          case Seq() => misQueries
          case _ =>
            misQueries map { _.toInt } filter { mismatches.contains(_) } map {
              _.toString
            }
        }
