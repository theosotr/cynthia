...
case class SQLAlchemyTranslator(target: Target) extends Translator {
  ...
  override def emitPrint(q: Query, dFields: Seq[String], ret: String) = {
    def _dumpField(v: String, fields: Iterable[String], ident: String = "") =
      fields map { as =>
        s"${ident}dump(getattr($v,'$as', None), '$as')"
      } mkString "\n"
    q match {
  ...
  def getSQLAlchemyFieldName(field: String, withAlias: Boolean = false) = {
    def _getSQLAlchemyFieldName(acc: String, segs: List[String]): String =
      segs match {
        case Nil      => acc
        case h :: Nil => acc + "." + h
        case h :: t =>
    ...
    segs match {
      case f :: Nil =>
        if (withAlias) Utils.quoteStr(f) else TUtils.toFieldVar(f)
  ...
  def translatePred(pred: Predicate): String = pred match {
    ...
    case Lte(k, e) =>
      (Str(getSQLAlchemyFieldName(k)) << " <= " << constructFieldExpr(e)).!
    case Contains(k, v) =>
      (Str(getSQLAlchemyFieldName(k)) << ".contains(" << Utils.quoteStr(
        Utils.escapeStr(v)
      ) << ", autoescape=True)").!
    case StartsWith(k, e) =>
      (Str(getSQLAlchemyFieldName(k)) << ".startswith(" << Utils.quoteStr(
        Utils.escapeStr(e)
      ) << ", autoescape=True)").!
    case EndsWith(k, e) =>
      (Str(getSQLAlchemyFieldName(k)) << ".endswith(" << Utils.quoteStr(
        Utils.escapeStr(e)
      ) << ", autoescape=True)").!
    case Not(pred) =>
    ...
    case spec =>
      (s.aggrs, target.db) match {
        case (Seq(_, _*), Postgres(_, _, _)) |
            (Seq(_, _*), Cockroachdb(_, _, _)) | (Seq(_, _*), MSSQL(_, _, _)) =>
          ""
        case _ =>
          (
            Str("order_by(") << (
              Utils.removeDups(spec) map { x =>
                x match {
                  case (k, Desc) =>
                    "desc(" + getSQLAlchemyFieldName(k, withAlias) + ")"
                  case (k, Asc) =>
                    "asc(" + getSQLAlchemyFieldName(k, withAlias) + ")"
                }
              } mkString ","
            ) << ")"
          ).!
  ...
  def constructPrimAggr(fexpr: FieldExpr, fprefix: String) = {
    val (field, op) = fexpr match {
      ...
      case Count(Some(field)) =>
        (constructFieldExpr(field, fprefix), "func.count")
  ...
  def constructFieldExpr(fexpr: FieldExpr, fprefix: String = ""): String =
    fexpr match {
      case F(f) =>
      ...
      case Constant(v, Quoted) =>
        "literal(" + Utils.quoteStr(Utils.escapeStr(v)) + ")"
      case _ =>
  ...
  def constructAggrPrefix(s: State, subquery: Boolean) = {
    ...
    val aggrs = TUtils.mapNonHiddenFields(
      aggrS,
    ...
    val qstr =
      "session.query(" + (aggrs mkString ", ") + ").select_from(" + s.source + ")"
    QueryStr(
      ...
      s.aggrs match {
        case Seq() | Seq(FieldDecl(Count(None), _, _, _)) => {
          val dFields = TUtils.mapNonHiddenFields(
            s.fields.values,
            { x => TUtils.toFieldVar(FieldDecl.as(x)) }
          )
    ...
    if (fields.isEmpty) QueryStr()
      ...
      fields.foldLeft(QueryStr()) {
        case (acc, FieldDecl(f, as, t, _)) => {
          val str =
            Str("type_coerce(") << constructFieldExpr(f) << ", " << getType(
              StringF
            ) <<
  ...
  def constructGroupBy(groupBy: Set[String]) = groupBy match {
    case Seq() => ""
    case _ =>
      "group_by(" + (groupBy map { x =>
        getSQLAlchemyFieldName(x)
      } mkString ", ") + ")"
  }
      ...
      target.db match {
        ...
        case _ =>
          throw new UnsupportedException(
            "Distinct on is supported only by Postgres"
          )
      }
    }
    case _ => ""
  ...
  override def constructNaiveQuery(
    ...
    val qStr =
      aliasStms >> constructFieldDecls(fieldVals) >> constructQueryPrefix(s)
    ...
    qStr >> QueryStr(
      ...
      Some(
        Seq(
          qStr.ret.get,
          constructJoins(s.joins.toSet.toSeq.sortWith { (x, y) =>
            x.size < y.size
          }),
