...
case class PeeweeTranslator(target: Target) extends Translator {
  ...
  def getPeeweeFieldName(field: String, withAlias: Boolean = false) = {
    def _getPeeweeFieldName(acc: String, segs: List[String]): String =
      segs match {
        case Nil      => acc
        case h :: Nil => acc + "." + h
        case h :: t =>
  ...
  def translatePred(pred: Predicate): String = pred match {
    ...
    case Lte(k, e) =>
      (Str(getPeeweeFieldName(k)) << " <= " << constructFieldExpr(e)).!
    case Contains(k, v) =>
      (Str(getPeeweeFieldName(k)) << ".contains(" << Utils.quoteStr(
        Utils.escapeStr(v)
      ) << ")").!
    case StartsWith(k, v) =>
      (Str(getPeeweeFieldName(k)) << ".startswith(" << Utils.quoteStr(
        Utils.escapeStr(v)
      ) << ")").!
    case EndsWith(k, v) =>
      (Str(getPeeweeFieldName(k)) << ".endswith(" << Utils.quoteStr(
        Utils.escapeStr(v)
      ) << ")").!
    case Not(pred) =>
          ...
          (
            Str("order_by(") << (
              spec map { x =>
                x match {
                  case (k, Desc) =>
                    getPeeweeFieldName(k, withAlias = withAlias) + ".desc()"
                  case (k, Asc) =>
                    getPeeweeFieldName(k, withAlias = withAlias) + ".asc()"
                }
              } mkString ","
            ) << ")"
          ).!
  ...
  def constructFieldExpr(fexpr: FieldExpr): String = fexpr match {
    ...
    case Constant(v, Quoted) =>
      s"Value(${Utils.quoteStr(Utils.escapeStr(v))}, converter=False)"
    case _ =>
      ...
      s.aggrs match {
        case Seq() | Seq(FieldDecl(Count(None), _, _, _)) => {
          ...
          val allFields = s.distinct match {
            case None => dFields
            case _ =>
              dFields ++ (s.orders map { case (x, _) =>
                getPeeweeFieldName(x, withAlias = false)
              })
          }
