...
case class ActiveRecordTranslator(target: Target) extends Translator {
  ...
  def getSelectFields(s: State, nonAggrHidden: Map[String, String]) = {
    ...
    s.distinct match {
      case Some(_) =>
        nonHiddenFieldsMap ++ s.orders
          .filter(x => !nonHiddenFieldsMap.contains(x._1))
          .map { case (s, _) =>
    ...
    def handlePredicate(
      ...
      if (!e.isConstant())
        throw new UnsupportedException(
          "complex where clauses are not supported"
        )
      ...
      if (symbol.equals("LIKE"))
        (
          key + s" $symbol ?",
          s""", "%#{${constructFieldExpr(e, fields, true)}}%""""
        )
      else
        (key + s" $symbol ?", ", " + constructFieldExpr(e, fields))
    }
    def handleWithPredicate(
        k: String,
        e: String,
        symbol: String
    ): (String, String) = {
      ...
      if (symbol.equals("StartsWith"))
        (
          key + s" LIKE ?",
          s""", "#{${Utils.quoteStr(escapeActiveRecordStr(e, true), "\"")}}%""""
        )
      else
        (
          key + s" LIKE ?",
  ...
  def constructFieldExpr(
    ...
    case Add(e1, e2) =>
      "(" + constructFieldExpr(e1, fields) + "+" + constructFieldExpr(
        e2,
        fields
      ) + ")"
    case Sub(e1, e2) =>
  ...
  def constructAggrExpr(fexpr: FieldExpr, fields: Map[String, String]) = {
    fexpr match {
      ...
      case Count(Some(field)) =>
        "count(\"" + constructFieldExpr(field, fields) + "\")"
      case Sum(field) => "sum(\"" + constructFieldExpr(field, fields) + "\")"
      case Avg(field) =>
        "average(\"" + constructFieldExpr(field, fields) + "\")"
      case Min(field) =>
        "minimum(\"" + constructFieldExpr(field, fields) + "\")"
      case Max(field) =>
        "maximum(\"" + constructFieldExpr(field, fields) + "\")"
      case _ =>
        throw new UnsupportedException(
          "compound aggrates not supported"
        ) // Revisit
    }
  }
  ...
  def constructField(acc: Map[String, String], fexpr: FieldExpr): String =
    fexpr match {
      case F(f) =>
        acc.get(f) match {
          case None    => f.split('.').takeRight(2).mkString(".").toLowerCase()
      ...
      case Add(e1, e2) =>
        "(" + constructField(acc, e1) + "+" + constructField(acc, e2) + ")"
      case Sub(e1, e2) =>
        "(" + constructField(acc, e1) + "-" + constructField(acc, e2) + ")"
      case Mul(e1, e2) =>
        "(" + constructField(acc, e1) + "*" + constructField(acc, e2) + ")"
    ...
    fields.foldLeft(List[String]()) { (acc, x) =>
      {
        ...
        acc :+ "select(" + Utils.quoteStr(
          s"${expr} as ${getDBAliasName(name)}",
          "\""
        ) + ")"
      }
    } mkString (".")
  ...
  def constructDistinct(distinct: Option[String]) = distinct match {
    case Some("") => "distinct"
    case Some(x) =>
      throw new UnsupportedException("Distincts with fields are not supported")
    case _ => ""
  }
  ...
  override def constructNaiveQuery(
    ...
    def filterNonHiddenFieldDecls(i: String) =
      !FieldDecl.hidden(s.fields.get(i).get)
    val model = s.source
    fieldsMap = extractFields(s.fields)
    nonHiddenFieldsMap =
      (fieldsMap.view filterKeys filterNonHiddenFieldDecls).toMap
