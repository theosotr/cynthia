  ...
  def constructCombinedQueries(s: State): QueryStr = s.from match {
    ...
    case Some(UnionState(s1, s2)) =>
      constructCombinedQuery(
        unionQueries(s1, s2).copy(aggrs = s.aggrs, orders = s.orders)
      )
    case Some(IntersectState(s1, s2)) =>
      constructCombinedQuery(
        intersectQueries(s1, s2).copy(aggrs = s.aggrs, orders = s.orders)
      )
  }
...
object TUtils {
  ...
  def filterNonAggrHidden(fields: Iterable[FieldDecl]) =
    filterMapAs({ x => !(FieldDecl.hidden(x) || FieldDecl.isAggregate(x)) })(
      fields
    )
    ...
    fields.foldLeft((Seq[String](), Seq[String]())) { (acc, x) =>
      {
        ...
        else if (!FieldDecl.isAggregate(x) && !FieldDecl.hidden(x)) (a, b :+ as)
        else (a, b)
      }
    }
}
