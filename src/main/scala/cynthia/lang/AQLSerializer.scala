...
object AQLJsonProtocol extends DefaultJsonProtocol {
  ...
  implicit object FieldExprJsonFormat extends RootJsonFormat[FieldExpr] {
    def write(fexpr: FieldExpr) = {
      val value = fexpr match {
        ...
        case Add(f1, f2) =>
          JsObject("Add" -> JsObject("f1" -> f1.toJson, "f2" -> f2.toJson))
        case Sub(f1, f2) =>
          JsObject("Sub" -> JsObject("f1" -> f1.toJson, "f2" -> f2.toJson))
        case Mul(f1, f2) =>
          JsObject("Mul" -> JsObject("f1" -> f1.toJson, "f2" -> f2.toJson))
        case Div(f1, f2) =>
          JsObject("Div" -> JsObject("f1" -> f1.toJson, "f2" -> f2.toJson))
      }
              ...
              fields.keys.toList.head match {
                ...
                case "Constant" =>
                  fields("Constant") match {
                    case JsObject(constant) =>
                      Constant(
                        constant("v").convertTo[String],
                        constant("vt").convertTo[ConstantType]
                      )
                ...
                case "Add" =>
                  fields("Add") match {
                    case JsObject(add) =>
                      Add(
                        add("f1").convertTo[FieldExpr],
                        add("f2").convertTo[FieldExpr]
                      )
                ...
                case "Sub" =>
                  fields("Sub") match {
                    case JsObject(sub) =>
                      Sub(
                        sub("f1").convertTo[FieldExpr],
                        sub("f2").convertTo[FieldExpr]
                      )
                ...
                case "Mul" =>
                  fields("Mul") match {
                    case JsObject(mul) =>
                      Mul(
                        mul("f1").convertTo[FieldExpr],
                        mul("f2").convertTo[FieldExpr]
                      )
                ...
                case "Div" =>
                  fields("Div") match {
                    case JsObject(div) =>
                      Div(
                        div("f1").convertTo[FieldExpr],
                        div("f2").convertTo[FieldExpr]
                      )
  ...
  implicit object PredicateJsonFormat extends RootJsonFormat[Predicate] {
    def write(predicate: Predicate) = {
      val value = predicate match {
        case Eq(k, f) =>
          JsObject("Eq" -> JsObject("k" -> JsString(k), "f" -> f.toJson))
        case Gt(k, f) =>
          JsObject("Gt" -> JsObject("k" -> JsString(k), "f" -> f.toJson))
        case Lt(k, f) =>
          JsObject("Lt" -> JsObject("k" -> JsString(k), "f" -> f.toJson))
        case Gte(k, f) =>
          JsObject("Gte" -> JsObject("k" -> JsString(k), "f" -> f.toJson))
        case Lte(k, f) =>
          JsObject("Lte" -> JsObject("k" -> JsString(k), "f" -> f.toJson))
        case Contains(k, f) =>
          JsObject("Contains" -> JsObject("k" -> JsString(k), "f" -> f.toJson))
        case StartsWith(k, f) =>
          JsObject(
            "StartsWith" -> JsObject("k" -> JsString(k), "f" -> f.toJson)
          )
        case EndsWith(k, f) =>
          JsObject("EndsWith" -> JsObject("k" -> JsString(k), "f" -> f.toJson))
        case And(p1, p2) =>
          JsObject("And" -> JsObject("p1" -> p1.toJson, "p2" -> p2.toJson))
        case Or(p1, p2) =>
          JsObject("Or" -> JsObject("p1" -> p1.toJson, "p2" -> p2.toJson))
              ...
              fields.keys.toList.head match {
                case "Eq" =>
                  fields("Eq") match {
                    case JsObject(values) =>
                      Eq(
                        values("k").convertTo[String],
                        values("f").convertTo[FieldExpr]
                      )
                ...
                case "And" =>
                  fields("And") match {
                    case JsObject(values) =>
                      And(
                        values("p1").convertTo[Predicate],
                        values("p2").convertTo[Predicate]
                      )
  ...
  implicit object FieldDeclTypeJsonFormat extends JsonFormat[FieldDecl] {
    def write(fd: FieldDecl) = fd match {
      case FieldDecl(f, as, ftype, hidden) =>
        JsObject(
          "FieldDecl" -> JsObject(
            "f" -> f.toJson,
            "as" -> as.toJson,
  ...
  implicit object QuerySetTypeJsonFormat extends JsonFormat[QuerySet] {
    def write(qs: QuerySet) = {
      val value = qs match {
        case New(m, f) =>
          JsObject("New" -> JsObject("m" -> m.toJson, "f" -> f.toJson))
        case Apply(op, q) =>
          JsObject("Apply" -> JsObject("op" -> op.toJson, "q" -> q.toJson))
        case Union(q1, q2) =>
          JsObject("Union" -> JsObject("q1" -> q1.toJson, "q2" -> q2.toJson))
        case Intersect(q1, q2) =>
          JsObject(
            "Intersect" -> JsObject("q1" -> q1.toJson, "q2" -> q2.toJson)
          )
      }
              ...
              fields.keys.toList.head match {
                case "New" =>
                  fields("New") match {
                    case JsObject(values) =>
                      New(
                        values("m").convertTo[String],
                        values("f").convertTo[Seq[FieldDecl]]
                      )
                ...
                case "Apply" =>
                  fields("Apply") match {
                    case JsObject(values) =>
                      Apply(
                        values("op").convertTo[Operation],
                        values("q").convertTo[QuerySet]
                      )
                ...
                case "Union" =>
                  fields("Union") match {
                    case JsObject(values) =>
                      Union(
                        values("q1").convertTo[QuerySet],
                        values("q2").convertTo[QuerySet]
                      )
  ...
  implicit object QueryTypeJsonFormat extends JsonFormat[Query] {
    def write(q: Query) = {
      val value = q match {
        ...
        case SubsetRes(offset, Some(limit), qs) =>
          JsObject(
            "SubsetRes" -> JsObject(
              "offset" -> offset.toJson,
              "limit" -> limit.toJson,
        ...
        case AggrRes(aggrs, qs) =>
          JsObject(
            "AggrRes" -> JsObject("aggrs" -> aggrs.toJson, "qs" -> qs.toJson)
          )
      }
              ...
              fields.keys.toList.head match {
                  ...
                  fields("SubsetRes") match {
                    case JsObject(values) =>
                      if (values.contains("limit"))
                        SubsetRes(
                          values("offset").convertTo[Int],
                          Some(values("limit").convertTo[Int]),
                          values("qs").convertTo[QuerySet]
                        )
                      else
                        SubsetRes(
                          values("offset").convertTo[Int],
                ...
                case "AggrRes" =>
                  fields("AggrRes") match {
                    case JsObject(values) =>
                      AggrRes(
                        values("aggrs").convertTo[Seq[FieldDecl]],
                        values("qs").convertTo[QuerySet]
                      )
                    case _ => deserializationError("FirstRes expected")
                  }
